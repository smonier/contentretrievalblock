...
<c:if test="${not empty bindedComponent && jcr:isNodeType(bindedComponent, type, 'crbnt:heading')}">
                    ...
                    <c:if test="${categories != null}">
                        <c:forEach items="${categories}" var="category">
                            <i class="fa fa-tag" aria-hidden="true"></i> <span
                                class="categorytitle mr-3">${category.node.displayableName} </span> </c:forEach>
